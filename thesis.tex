...
Now, it will take time 2 for each player to reach the terminal. Thus, under strategic interaction we see that the equilibrium is sub-optimal for society, and in this case all players as well. So, the ratio of the social welfare (total costs to all players) for the worst possible equilibria compared to the best possible (but not necessarily equilibria) constructed can be computed $$\frac{\sum_{n=1}^{N} n \cdot 2}{\sum_{n=1}^N n \cdot 1.5} = \frac{4}{3}$$ where we sum the cost for our $N$ players in each of the two scenarios described above depicted in figure \ref{braess2}, and figure \ref{braess3} \citep{Roughgarden2017}. For this network, there is only one (Nash) equilibrium for selfish routing games, however, price of anarchy bounds can be found for the class of game as a whole regardless of its individual construction. These bounds tell us how much worse then optimal we can expect a system at equilibrium to behave in the worst case. The price of anarchy bound for these selfish routing games with affine cost functions (i.e. $c(x) = ax+b$) is know to be $4/3$, meaning that our example is as bad as the ratio between the best possible social welfare and the worst possible equilibria could get for any network to play this game \citep{Roughgarden2007}. 

\subsection{Auctions}
The lens of the price of anarchy is now being applied to understand auctions by researchers in the algorithmic game theory. Where for selfish routing games, the price of anarchy has been shown to have the tight bound above, the price of anarchy for single price auctions only has approximate upper and lower bounds set, and is not known at all for double auctions. Before exploring this further, we should set up the mathematical framework of auctions that is necessary to understand the social welfare in auctions, as well as how strategic interaction arises. 
